...
{
    static lua_gc_fn real_function = reinterpret_cast<lua_gc_fn>(resolveSharedSymbolOrExit(kLibLuaName, "lua_gc"));
...
{
    lua_gc_fn real_function = resolve_lua_gc();

    if (operation == LUA_GCSTEP)
    {
        size_t step_size = va_arg(args, size_t);
        return real_function(state, operation, step_size);
    }
    if (operation == LUA_GCPARAM)
    {
        int parameter = va_arg(args, int);
        int value = va_arg(args, int);
        return real_function(state, operation, parameter, value);
    }
    return real_function(state, operation);
}
